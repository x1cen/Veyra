...
//                        cursor = database.queryFinalized(String.format(Locale.US, "SELECT data, mid, date, uid FROM messages_v2 WHERE mid IN(%s) AND uid = %d", TextUtils.join(",", ids), dialogId));
                    ...
                    while (cursor.next()) {
                        ...
                        if (data != null) {
                            ...
                            if (!quickReplies && !findInScheduled) message.isDeleted = !cursor.isNull(4);
    ...
    public void deleteDialog(long did, int messagesOnly) {
        storageQueue.postRunnable(() -> {
            ...
            try {
                ...
                database.executeFast("DELETE FROM veyra_message_deletions WHERE uid = " + did).stepThis().dispose();
...
//                }
                ...
                for (int a = 0, N = messagesByDialogs.size(); a < N; a++) {
                    ...
                    database.executeFast(String.format(Locale.US, "DELETE FROM veyra_message_deletions WHERE mid IN(%s) AND uid = %d", ids, did)).stepThis().dispose();
    ...
    private List<Long> markMessagesAsIsDeletedInternal(Long dialogId, List<Integer> messages) {
        try {
            SQLiteCursor cursor;
            String ids = TextUtils.join(",", messages);
    ...
    public List<Long> markMessagesAsIsDeleted(Long dialogId, List<Integer> messages, boolean useQueue) {
    ...
    public void updateDialogsWithDeletedMessages(long dialogId, long channelId, List<Integer> messages, List<Long> additionalDialogsToUpdate) {
